...
class Node:
    """Classe auxiliar para manter a estrutura da árvore de busca."""
...
def solve(initial_state, max_memory=5000):
    """LMA* com limite de memória melhorado (H2 - Distância)."""
    start_time = time.time()
    root = Node(initial_state, None, None, 0, heuristic_h2(initial_state))
    ...
    while open_list:
        ...
        if best_node.state.is_goal():
            ...
            while curr.parent:
                ...
                curr = curr.parent
            return {
                "name": f"LMA* com Heurística de Distância (H2) - Memória: {max_memory}",
